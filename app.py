...
# --- Tambahan Import untuk Merapikan Tabel Excel ---
...
ARTICLE_STATUSES = [
    "New Submission", "Desk Review", "Assign Reviewer", "Under Review",
    "Review Feedback Received", "Revision Requested", "Revision Submitted",
    "Accepted", "APC Pending", "Payment Proof Submitted", "Payment Verified",
    "Ready Copy Edit", "Copy Editing", "Layout Editing", "Proofreading",
    "Scheduled", "Published", "Rejected", "Withdrawn",
]

REVIEWER_STATUSES = ["Invited", "Accepted", "Declined", "Review Submitted", "Unresponsive"]
REVIEW_RECOMMENDATIONS = ["", "Accept", "Minor Revision", "Major Revision", "Reject"]
REVIEWER_CATEGORIES = ["Nasional", "Binus"]

FOLDER_STRUCTURE = [
    "01_Submission", "02_Desk_Review", "03_Reviewers", "04_Review_Results",
    "05_Decision_Letters", "06_Revisi_Author", "07_APC", "08_Copyediting",
    "09_Layout_Proof", "10_Published",
]
...
ALLOWED_UPLOAD_EXTENSIONS = ["doc", "docx", "pdf", "xlsx", "xls", "csv", "png", "jpg", "jpeg", "zip", "txt"]

UPLOAD_FOLDER_RULES = {
    "01_Submission": ["manuskrip", "format form", "format_form", "turnitin"],
    "02_Desk_Review": ["desk review", "desk_review", "revisi desk review", "revisi_desk_review"],
...
def inject_dashboard_theme():
    ...
    h1, h2, h3, h4, h5, h6 { color: #6f42c1 !important; }
    [data-testid="stMetric"] { background-color: var(--secondary-background-color); padding: 1rem; border-radius: 0.5rem; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-left: 4px solid #6f42c1; }
    [data-testid="stDataFrame"], .stDataFrame { background-color: var(--background-color); border-radius: 0.5rem; padding: 1rem; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
    .stButton>button[kind="primary"] { background-color: #6f42c1; color: white; border: none; }
    .stButton>button[kind="primary"]:hover { background-color: #59339d; }
    [data-testid="stSidebar"] div[role="radiogroup"] > label { border: 1px solid var(--primary-color); border-radius: 8px; padding: 12px 15px; margin-bottom: 8px; background-color: var(--secondary-background-color); transition: all 0.2s ease-in-out; cursor: pointer; }
    [data-testid="stSidebar"] div[role="radiogroup"] > label:hover { border-color: #6f42c1; background-color: rgba(111, 66, 193, 0.1); transform: translateY(-2px); box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
    [data-testid="stSidebar"] div[role="radiogroup"] > label p { font-size: 1.15rem !important; font-weight: 500 !important; }
    </style>
    """, unsafe_allow_html=True)


# =========================================================
# DATABASE & LOGBOOK SEEDER
# =========================================================
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS articles (
            id INTEGER PRIMARY KEY AUTOINCREMENT, article_code TEXT UNIQUE NOT NULL, journal_code TEXT NOT NULL,
            submission_id TEXT, submission_link TEXT, title TEXT NOT NULL, corresponding_author TEXT, author_email TEXT, affiliation_1 TEXT, affiliation_2 TEXT,
            status TEXT NOT NULL DEFAULT 'New Submission', submission_date TEXT, desk_review_deadline TEXT, author_revision_deadline TEXT,
            author_reminder_count INTEGER DEFAULT 0, apc_deadline TEXT, payment_proof_date TEXT, payment_verified_date TEXT,
            volume TEXT, issue TEXT, publication_year TEXT, folder_path TEXT, notes TEXT, is_international INTEGER DEFAULT 0,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP
        )
    """)
    
    # Migrasi database jika kolom baru belum ada
    try: cur.execute("ALTER TABLE articles ADD COLUMN is_international INTEGER DEFAULT 0")
    except sqlite3.OperationalError: pass
    
    try: cur.execute("ALTER TABLE articles ADD COLUMN submission_link TEXT")
    except sqlite3.OperationalError: pass

    cur.execute("""
        CREATE TABLE IF NOT EXISTS reviewers (
            id INTEGER PRIMARY KEY AUTOINCREMENT, article_id INTEGER NOT NULL, reviewer_name TEXT NOT NULL, reviewer_email TEXT,
            reviewer_category TEXT NOT NULL DEFAULT 'Nasional', invitation_date TEXT, invitation_deadline TEXT, acceptance_date TEXT,
            review_deadline TEXT, review_submitted_date TEXT, status TEXT NOT NULL DEFAULT 'Invited', recommendation TEXT,
            reminder_count INTEGER DEFAULT 0, notes TEXT, FOREIGN KEY(article_id) REFERENCES articles(id)
        )
    """)

    cur.execute("""
        CREATE TABLE IF NOT EXISTS communication_log (
            id INTEGER PRIMARY KEY AUTOINCREMENT, article_id INTEGER NOT NULL, reviewer_id INTEGER, communication_date TEXT NOT NULL,
            recipient_type TEXT NOT NULL, recipient_name TEXT, communication_type TEXT NOT NULL, subject TEXT, notes TEXT,
            FOREIGN KEY(article_id) REFERENCES articles(id), FOREIGN KEY(reviewer_id) REFERENCES reviewers(id)
        )
    """)

    cur.execute("CREATE TABLE IF NOT EXISTS contacts (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT, affiliation TEXT, role TEXT, UNIQUE(name, email))")
    ...
    # Jalankan seeder logbook otomatis
    seed_logbook_data()

def seed_logbook_data():
    """Mengisi logbook otomatis dari data history jika tabel masih kosong"""
    conn = get_connection()
    count = conn.execute("SELECT COUNT(*) FROM logbook").fetchone()[0]
    
    if count == 0:
        raw_data = """Kegiatan|Volume|Satuan|Tanggal|Status
Copy Editing (Lingua Cultura 14444)|1|Artikel|2026-07-07|Done
Cek email (4)|4|Email|2026-07-07|Done
Copy Editing (Lingua Cultura 14593)|1|Artikel|2026-07-07|Done
Desk Review (ComTech 15455)|1|Artikel|2026-07-07|Done
Copy Editing (Lingua Cultura 14585)|1|Artikel|2026-07-07|Done"""
        
        insert_data = []
        for line in raw_data.strip().split('\n')[1:]:
            parts = line.split('|')
            if len(parts) == 5:
                insert_data.append((parts[0], int(parts[1]), parts[2], parts[3], "", "", parts[4]))
                
        conn.executemany("""
            INSERT INTO logbook (kegiatan, volume, satuan, tanggal, jam_mulai, jam_selesai, status)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, insert_data)
        conn.commit()
    conn.close()
...
def build_excel_export():
    ...
    query = """
        SELECT
            a.article_code AS "Nomor", a.journal_code AS "Jurnal", 
            a.submission_id AS "Submission ID", a.submission_link AS "Link Submission",
...
def dataframe_to_excel_bytes(df):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        worksheet = writer.sheets["Monitoring All"]
        
        num_rows = len(df) + 1
        num_cols = len(df.columns)
        
        if num_cols > 0:
            ref_rows = num_rows if num_rows > 1 else 2
            table_ref = f"A1:{get_column_letter(num_cols)}{ref_rows}"
            
            tab = Table(displayName="MonitoringTable", ref=table_ref)
            ...
            for i, col in enumerate(df.columns, 1):
                ...
                max_len = max((len(str(val)) for val in df[col].dropna()), default=0)
                header_len = len(str(col))
                
                final_width = min(max(max_len, header_len) + 4, 50)
                worksheet.column_dimensions[col_letter].width = max(12, final_width)
...
def page_dashboard():
    ...
    with st.container(key="metrics_container"):
        ...
        if weekday <= 4:
            ...
            if not articles.empty:
                ...
                if not recommended_df.empty:
                    ...
                    # Konfigurasi Link submission pada tabel
                    display_recom = recommended_df[["article_code", "title", "status", "updated_at", "submission_link"]].copy()
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "submission_link": st.column_config.LinkColumn("Akses Halaman", display_text="🔗 Buka OJS")
                        }
                    )
                else:
    ...
    with tab_month:
        ...
        cal_str = [[str(day) if day != 0 else "" for day in week] for week in cal]
        df_cal = pd.DataFrame(cal_str, columns=["Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu", "Minggu"])
    ...
    with st.container(key="tasks_container"):
        if not tasks:
...
def page_add_article():
    ...
    with st.form("form_add_article", clear_on_submit=True):
        ...
        with col1:
            ...
            submission_link = st.text_input("Link Halaman Submission (Opsional)", placeholder="Contoh: https://journal.binus.ac.id/...") # FITUR BARU
        ...
        create_folder = st.checkbox("Buat struktur folder otomatis", value=True)
        ...
        try:
            ...
            conn.execute("""
                INSERT INTO articles (
                    article_code, journal_code, submission_id, submission_link, title, corresponding_author, author_email,
                    affiliation_1, affiliation_2, status, submission_date, folder_path, notes, is_international, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                clean_article_code, journal_code, submission_id.strip(), submission_link.strip(), title.strip(), corresponding_author.strip(),
...
def page_manage_articles():
    ...
    # Tambahkan kolom submission link ke dalam tabel manajemen agar bisa di-klik juga
    display_columns = ["article_code_display", "journal_code", "title", "corresponding_author", "status", "reviewer_count", "author_revision_deadline", "submission_link"]
    renamed_df = display_df[display_columns].rename(columns={
        ...
        "status": "Status", "reviewer_count": "Reviewer", "author_revision_deadline": "Deadline Revisi", "submission_link": "Link"
    })
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Link": st.column_config.LinkColumn("Link Akses", display_text="🔗 Buka")
        }
    )
    ...
    with tabs[0]:
        ...
        with st.form(f"form_article_status_{article['id']}"):
            ...
            with col1: 
                ...
                submission_link_edit = st.text_input("Link Halaman Submission (OJS)", value=article.get("submission_link", "")) # FITUR BARU
            with col2: 
            ...
            if st.form_submit_button("Simpan Perubahan", type="primary"):
                ...
                conn.execute("""
                    UPDATE articles
                    SET status=?, volume=?, issue=?, publication_year=?, notes=?, is_international=?, submission_link=?, updated_at=?
                    WHERE id=?
                """, (status, volume.strip(), issue.strip(), publication_year.strip(), notes.strip(), 1 if is_international else 0, submission_link_edit.strip(), datetime.now().isoformat(timespec="seconds"), article["id"]))
                conn.commit()
                conn.close()
...
# =========================================================
# HALAMAN LOGBOOK AGENDA HARIAN
# =========================================================

def extract_base_kegiatan(kegiatan):
    """Menghilangkan nomor artikel dari string kegiatan, contoh: 'Copy Editing (CT 1444)' -> 'Copy Editing'"""
    return re.split(r' \(', str(kegiatan))[0].strip()

def logbook_to_excel(df):
    """Fungsi untuk merapikan format Excel khusus Logbook (Format Tabel Resmi) agar mudah dibaca"""
    output = BytesIO()
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_cleaned.to_excel(writer, index=False, sheet_name='Logbook Harian')
        worksheet = writer.sheets['Logbook Harian']
        ...
        if num_cols > 0:
            ...
            tab = Table(displayName="LogbookTable", ref=table_ref)
            style = TableStyleInfo(name="TableStyleMedium9", showFirstColumn=False,
                                   showLastColumn=False, showRowStripes=True, showColumnStripes=False)
            tab.tableStyleInfo = style
            worksheet.add_table(tab)
            ...
            for i, col in enumerate(df_cleaned.columns, 1):
                col_letter = get_column_letter(i)
                max_length = max((len(str(val)) for val in df_cleaned[col].dropna()), default=0)
                header_length = len(str(col))
                
                final_width = min(max(max_length, header_length) + 4, 60)
                worksheet.column_dimensions[col_letter].width = max(15, final_width)
                
                for cell in worksheet[col_letter]:
                    cell.alignment = Alignment(vertical="center", wrap_text=(final_width >= 60))
...
def page_logbook():
    st.subheader("📒 Logbook Agenda Harian")
    
    col_top1, col_top2 = st.columns(2)
    
    with col_top1:
        with st.expander("📝 To-Do List Harian", expanded=True):
            col_td1, col_td2 = st.columns([3, 1])
            with col_td1:
                new_todo = st.text_input("Tambah Tugas Baru", placeholder="Ketik tugas di sini...", label_visibility="collapsed", key="todo_input_main")
            with col_td2:
                if st.button("➕ Tambah", use_container_width=True, key="btn_add_todo_main"):
                    if new_todo.strip():
                        conn = get_connection()
                        conn.execute("INSERT INTO todo_list (task) VALUES (?)", (new_todo.strip(),))
                        conn.commit()
                        conn.close()
                        st.rerun()

            conn = get_connection()
            todos = pd.read_sql_query("SELECT * FROM todo_list ORDER BY is_done ASC, id DESC", conn)
            conn.close()
            
            if todos.empty:
                st.caption("Belum ada tugas di To-Do List.")
            else:
                for _, row in todos.iterrows():
                    c1, c2, c3 = st.columns([0.1, 0.8, 0.1])
                    with c1:
                        is_checked = st.checkbox("", value=bool(row['is_done']), key=f"todo_{row['id']}")
                        if is_checked != bool(row['is_done']):
                            conn = get_connection()
                            conn.execute("UPDATE todo_list SET is_done = ? WHERE id = ?", (int(is_checked), row['id']))
                            conn.commit()
                            conn.close()
                            st.rerun()
                    with c2:
                        if row['is_done']:
                            st.markdown(f"<span style='text-decoration: line-through; color: gray;'>{row['task']}</span>", unsafe_allow_html=True)
                        else:
                            st.write(row['task'])
                    with c3:
                        if st.button("❌", key=f"del_todo_{row['id']}", help="Hapus dari daftar"):
                            conn = get_connection()
                            conn.execute("DELETE FROM todo_list WHERE id = ?", (row['id'],))
                            conn.commit()
                            conn.close()
                            st.rerun()

    with col_top2:
        with st.expander(f"📌 Kegiatan Tercatat Hari Ini ({date.today().strftime('%d %B %Y')})", expanded=True):
            conn = get_connection()
            today_str = date.today().isoformat()
            df_today = pd.read_sql_query("SELECT id, kegiatan, volume, satuan, status FROM logbook WHERE tanggal = ? ORDER BY id DESC", conn, params=(today_str,))
            conn.close()
            
            if df_today.empty:
                st.info("Belum ada kegiatan yang diinput untuk hari ini.")
            else:
                st.caption(f"Total tercatat hari ini: **{len(df_today)} aktivitas**")
                st.dataframe(
                    df_today[["kegiatan", "volume", "satuan", "status"]].rename(columns={"kegiatan": "Nama Kegiatan", "volume": "Vol", "satuan": "Satuan", "status": "Status"}),
                    use_container_width=True,
                    hide_index=True,
                    height=200
                )
    ...
    tab_input, tab_data, tab_viz = st.tabs(["📝 Input Logbook", "📋 Keseluruhan Data & Edit Logbook", "📊 Visualisasi"])
    ...
    with tab_input:
        with st.form("form_logbook", clear_on_submit=True):
            ...
            with c1:
                ...
                kegiatan = st.text_input("Kegiatan (Contoh: Copy Editing CT 1234)", placeholder="Tuliskan nama kegiatan/pekerjaan")
            with c2:
                col_vol, col_sat = st.columns(2)
                with col_vol: volume = st.number_input("Volume", min_value=1, value=1)
                with col_sat: satuan = st.selectbox("Satuan", ["Artikel", "Email", "Acara", "Tugas Khusus"])
                status = st.selectbox("Status", ["Done", "On Going"])
            
            if st.form_submit_button("Simpan ke Logbook", type="primary"):
                if not kegiatan.strip():
                    st.error("Nama kegiatan harus diisi.")
                else:
                    conn = get_connection()
                    conn.execute("""
                        INSERT INTO logbook (kegiatan, volume, satuan, tanggal, jam_mulai, jam_selesai, status)
                        VALUES (?, ?, ?, ?, '', '', ?)
                    """, (kegiatan.strip(), volume, satuan, tanggal.isoformat(), status))
                    conn.commit()
                    conn.close()
                    st.success("Tugas berhasil dicatat ke Logbook!")
                    st.rerun()

    conn = get_connection()
    df_log = pd.read_sql_query("SELECT id, tanggal, kegiatan, volume, satuan, status FROM logbook ORDER BY tanggal DESC, id DESC", conn)
    conn.close()
    ...
    with tab_data:
        if df_log.empty:
            st.info("Logbook kosong.")
        else:
            col_d1, col_d2 = st.columns([0.7, 0.3])
            with col_d1:
                st.markdown("Ini adalah jendela terpisah untuk melihat **keseluruhan riwayat kegiatan**. Anda bisa mengedit data secara langsung atau menghapus baris, lalu klik **Simpan Perubahan**.")
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    "id": None,
                    "tanggal": st.column_config.DateColumn("Tanggal (YYYY-MM-DD)", format="YYYY-MM-DD"),
                    "kegiatan": st.column_config.TextColumn("Nama Kegiatan"),
                    "volume": st.column_config.NumberColumn("Volume"),
                    "satuan": st.column_config.SelectboxColumn("Satuan", options=["Artikel", "Email", "Acara", "Tugas Khusus"]),
                    "status": st.column_config.SelectboxColumn("Status", options=["Done", "On Going"])
                }
            )
            ...
            with c_btn1:
                if st.button("💾 Simpan Perubahan Data", type="primary"):
                    ...
                    for _, r in edited_df.iterrows():
                        if pd.notna(r['id']):
                            tgl_str = r['tanggal'].isoformat() if pd.notna(r['tanggal']) else ""
                            conn.execute("UPDATE logbook SET tanggal=?, kegiatan=?, volume=?, satuan=?, status=? WHERE id=?",
                                         (tgl_str, r['kegiatan'], r['volume'], r['satuan'], r['status'], r['id']))
                    
                    existing_ids = edited_df['id'].dropna().tolist()
                    if existing_ids:
                        placeholders = ','.join('?' for _ in existing_ids)
                        conn.execute(f"DELETE FROM logbook WHERE id NOT IN ({placeholders})", existing_ids)
                    else:
            ...
            with c_btn2:
                ...
                df_export.rename(columns={"tanggal": "Tanggal", "kegiatan": "Nama Kegiatan", "volume": "Volume", "satuan": "Satuan", "status": "Status"}, inplace=True)
                
                st.download_button(
                    label="📥 Unduh Excel Logbook Harian (Format Rapi)",
                    data=logbook_to_excel(df_export),
                    file_name=f"Logbook_Harian_Iqbal_{date.today().isoformat()}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

    with tab_viz:
        if df_log.empty:
            st.info("Belum ada data di Logbook untuk divisualisasikan.")
        else:
            c_tot, c_done, c_ongo = st.columns(3)
            c_tot.metric("Total Aktivitas Tercatat", len(df_log))
            c_done.metric("Pekerjaan Selesai (Done)", len(df_log[df_log["status"] == "Done"]))
            c_ongo.metric("Sedang Dikerjakan (On Going)", len(df_log[df_log["status"] == "On Going"]))
            
            st.markdown("---")
            col_chart1, col_chart2 = st.columns(2)
            
            with col_chart1:
                st.markdown("#### 🏆 Tugas Paling Sering Dilakukan")
                df_log['Base Kegiatan'] = df_log['kegiatan'].apply(extract_base_kegiatan)
                top_tasks = df_log['Base Kegiatan'].value_counts().head(10)
                st.bar_chart(top_tasks)

            with col_chart2:
                st.markdown("#### 📅 Aktivitas Harian (Berdasarkan Waktu)")
                daily_counts = df_log.groupby("tanggal").size()
                st.bar_chart(daily_counts)
                
            st.markdown("#### 📊 Distribusi Kategori Satuan Kerja")
            satuan_counts = df_log['satuan'].value_counts()
            st.bar_chart(satuan_counts)
...
# =========================================================
# HALAMAN ANALISIS REVIEWER & AUTHOR 
# =========================================================
...
# =========================================================
# HALAMAN DASHBOARD KPI (INTEGRASI EXCEL)
# =========================================================

def page_kpi_dashboard():
    st.subheader("🎯 Real-Time KPI Performance Dashboard")
    st.markdown("Menampilkan data performa KPI Anda secara otomatis dari file `Dashboard_KPI_RealTime_Iqbal_V2.xlsx`.")

    file_path = "Dashboard_KPI_RealTime_Iqbal_V2.xlsx"
    
    if not os.path.exists(file_path):
        st.error(f"File excel `{file_path}` tidak ditemukan. Pastikan file sudah dipindahkan ke folder yang sama dengan aplikasi.")
        return
    ...
    try:
        ...
        if "Dashboard" in xls.sheet_names:
            df_dash = pd.read_excel(xls, sheet_name="Dashboard", nrows=5)
            try:
                info_text = str(df_dash.iloc[0, 1])
                if "Name:" in info_text:
                    st.info(f"👤 **Profil Karyawan:** {info_text}")
            except: pass

        kpi_data = []
        for sheet in xls.sheet_names:
            if sheet.startswith("KPI_"):
                df_kpi = pd.read_excel(xls, sheet_name=sheet)
                kpi_col_name = str(df_kpi.columns[0])
                
                try:
                    bobot_target = str(df_kpi.iloc[0, 0])
                    capaian = df_kpi.iloc[2, 1]
                    skor = df_kpi.iloc[3, 1]
                    
                    kpi_number = kpi_col_name.split(':')[0]
                    kpi_title = kpi_col_name.split(':')[1].strip() if ':' in kpi_col_name else kpi_col_name
                    
                    kpi_data.append({
                        "KPI": kpi_number,
                        "Indikator Kinerja": kpi_title,
                        "Bobot & Target": bobot_target.replace("Bobot: ", "").replace(" | Target: ", " | "),
                        "Capaian": capaian,
                        "Skor": skor
                    })
                except Exception:
        ...
        if kpi_data:
            df_summary = pd.DataFrame(kpi_data)
            df_summary["Capaian"] = pd.to_numeric(df_summary["Capaian"], errors='coerce').round(4)
            df_summary["Skor"] = pd.to_numeric(df_summary["Skor"], errors='coerce').fillna(0).astype(int)

            avg_score = df_summary["Skor"].mean()
            total_kpi = len(df_summary)
            kpi_max_score = len(df_summary[df_summary["Skor"] == 6])
            
            with st.container(key="kpi_metrics_container"):
                col1, col2, col3 = st.columns(3)
                col1.metric("Total Indikator KPI", total_kpi)
                col2.metric("Rata-Rata Skor", f"{avg_score:.2f} / 6.00")
                col3.metric("KPI dgn Skor Maksimal (6)", kpi_max_score)
            
            st.markdown("---")
            st.markdown("#### 📋 Rincian Capaian per KPI")
            st.dataframe(df_summary, use_container_width=True, hide_index=True)

            st.markdown("---")
            st.markdown("#### 📊 Visualisasi Skor KPI")
            chart_data = df_summary[["KPI", "Skor"]].set_index("KPI")
            st.bar_chart(chart_data)
            
        else:
            st.warning("Data KPI belum dapat diekstrak. Pastikan isi dan format Excel sesuai.")
...
def main():
    ...
    MENU_OPTIONS = {
        ...
        "🎯 Dashboard KPI": page_kpi_dashboard,
        "📒 Logbook Harian": page_logbook,  
